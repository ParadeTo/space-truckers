...
function createPlanet(opts, scene) {
  ...
  const tn = new TransformNode(planet.name + '-tn', scene)
  planet.parent = tn
  tn.position.x = opts.posRadius * Math.sin(opts.posRadians)
  tn.position.z = opts.posRadius * Math.cos(opts.posRadians)
  planet.tn = tn
  planet.material = mat
...
function createAndStartOrbitAnimation(planet, scene) {
  ...
  let preRenderObsv = scene.onBeforeRenderObservable.add((sc) => {
    planet.tn.position.x = opts.posRadius * Math.sin(angPos)
    planet.tn.position.z = opts.posRadius * Math.cos(angPos)
    // tn.rotation.y = angPos
